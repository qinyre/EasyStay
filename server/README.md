...
### 数据导入
- **导入 Mock 数据**：`node importMockData.js`
  - 导入 5 家精选酒店数据
  - 包含完整的房型、设施、描述信息
  - 自动创建测试商户账号

### 数据迁移
- **JSON 到 SQLite 迁移**：`node scripts/migrateData.js`
  - 将旧的 `data/hotels.json` 和 `data/users.json` 迁移到 SQLite
  - 自动处理密码加密
